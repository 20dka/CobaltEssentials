...
local function init(configPort)
	...
	server:setpeername('192.168.0.11', tonumber(port))


	server:send(json.stringify({event='ping'}))
	local resp, err = server:receive()
	if (not resp) or err then
		CElog('CobaltDB connection failed: '..err, 'WARN')
		socket.sleep(0.2)
		return false
	else
		CElog('CobaltDB response looks ok', 'CobaltDB')
		return true
	end
end
...
local function newDatabase(DBname)
	...
	--local jsontosend = 
	--print("sending:",(jsontosend:gsub('\n',''):gsub('\t','')))
	server:send(json.stringify({event = "openDatabase", dbname = DBname} ))
	
	local databaseLoaderInfo = server:receive()
	--print("received:", databaseLoaderInfo)
...
local function set(DBname, tableName, key, value)
	...
	server:send(json.stringify({event = "set", dbname = DBname, table = tableName, key = key, value = value} ))
end
...
local function query(DBname, tableName, key)
	...
	server:send(json.stringify({event = "query", dbname = DBname, table = tableName, key = key} ))
...
local function getTable(DBname, tableName)
	...
	server:send(json.stringify({event = "getTable", dbname = DBname, table = tableName} ))
...
local function getTables(DBname)
	...
	server:send(json.stringify({event = "getTables", dbname = DBname} ))
...
local function getKeys(DBname, tableName)
	...
	server:send(json.stringify({event = "getKeys", dbname = DBname, table = tableName} ))
...
local function tableExists(DBname, tableName)
	...
	server:send(json.stringify({event = "tableExists", dbname = DBname, table = tableName} ))
